...
local grep_default_exe, grep_default_opt, grep_default_ropt, grep_default_expr_opt, grep_default_fix_string_opt, grep_default_ignore_case, grep_default_smart_case =
  require('todo.search').default_tool()

local logger = require('todo.logger')
...
local function update_todo_content() -- {{{
  ...
  then
    extend(argv, { '.' })
  elseif util.isWindows() and grep_default_exe == 'findstr' then
    extend(argv, { '*.*' })
  elseif not util.isWindows() and grep_default_exe == 'rg' then
    extend(argv, { '.' })
  end
...
function M.list() -- {{{
  if not project_manager_registered then
    local ok, r = pcall(require, 'rooter')
    if ok then
      r.reg_callback(M.on_cwd_changed, 'update_todo_content')
